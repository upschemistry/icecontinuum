    ...
    def run(self, print_progress=True, print_count_layers=False, halve_time_res=False) -> None:
        ...
        if self.nonstd_init:
            ...
            if self.dimension == 0:
                ...
                Fliq = df.getNliq_array(Nice,Nstar,Nbar,niter)
                sigma = df.getsigmastep(self.x, np.max(self.x), self.center_reduction, self.sigmastepmax)
                model_args = (packed_float_params,sigma)
                nliq_func = df.getNliq_array # function to get to update fliq
            elif self.dimension == 2:
                Fliq = df.getNliq_2d_array(Nice,Nstar,Nbar,niter) # Initialize as a pre-equilibrated layer of liquid over ice
                sigma = df.getsigmastep_2d(self.x,self.y, self.center_reduction, self.sigmastepmax) # supersaturation
                model_args = (packed_float_params,sigma) 
                nliq_func = df.getNliq_2d_array # function to get to update fliq
            Ntot = Fliq + Nice

        ## Y0 IS FIRST DEFINED HERE THEN COPIED INTO YLAST
        if self.dimension == 0:
...
def copy_sim(simulation: SimulationNew):
    #copy simulation params
    new_sim = SimulationNew(simulation.model, simulation.shape, method=simulation.method, rtol=simulation.rtol)
